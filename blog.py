"""
Original blog plugin, with a couple of improvements: it uses config, and puts pretty links into paths.
"blog": {
    "path": "blog",
    "body": "body",
    "author": "author",
...
def preBuild(site):
    ...
    blogBody = blog.get('body', 'body')
    dateFormat = blog.get('date-format', '%d-%m-%Y')
    blogAuthor = blog.get('author', 'Unknown')
    ...
    for page in site.pages():
        if page.path.startswith(blogPath):
            ...
            postContext['author'] = find('author') or blogAuthor
            postContext['date'] = find('date')
            postContext['path'] = page.final_url
            postContext[blogBody] = getNode(get_template(page.path), name=blogBody)
